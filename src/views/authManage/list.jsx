...
export default function List() {
    ...
    const columns = [
      ...
      },
      {
          ...
          render: item => {
              return (
                  <div>
                      <Button danger shape="circle" icon={<DeleteOutlined />} onClick={() => showConfirm(item)}  style={{ marginRight: "10px" }}/>
                      <Popover content={
  ...
  const changeSwitchCheck = item => {
    ...
    if (item.grade === 1) {
      React.$http.patch(`/rights/${item.id}`, {
            pagepermisson: item.pagepermisson
        }).then(() => {
            setDataSource([...dataSource])
        })
    } else {
      React.$http.patch(`/children/${item.id}`, {
            pagepermisson: item.pagepermisson
        }).then(() => {
            setDataSource([...dataSource])
        })
...
const deleteRightList = item => {
    if (item.grade === 1) {
      React.$http.delete(`/rights/${item.id}`).then(() => {
        ...
        React.$http.delete(`/children/${item.id}`).then(() => {
            setDataSource([...dataSource])
        })
