...
import { SAVEUSER } from '../../util/constant.js'
...
export default function Home() {
    ...
    useEffect(() => {
      React.$http("/news?publishState=2&_expand=category&_sort=view&_order=desc&_limit=6").then((res) => {
            //console.log(res.data);
            setViewList(res)
        })

        React.$http("/news?publishState=2&_expand=category&_sort=star&_order=desc&_limit=6").then((res) => {
            //console.log(res.data);
            setStarList(res)
        })
    }, [])
    ...
    useEffect(() => {
      React.$http("/news?publishState=2&_expand=category").then((res) => {
            //console.log(res.data);

            //第一个参数为要操作的数据，第二个参数为函数，返回要提取的字段
            //console.log(_.groupBy(res.data, item => item.category.title));
            const pieList = res.filter(item => item.author === SAVEUSER().username)
            
            showCharts(_.groupBy(res, item => item.category.title))
            setAllList(pieList)
        })
        //当组件销毁后不再关注窗口大小
        return () => {
