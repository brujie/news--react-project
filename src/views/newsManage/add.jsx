...
export default function NewsAdd(props) {
    ...
    useEffect(() => {
        if (props.match.params.id) {
            React.$http.get(`/news/${props.match.params.id}?_expand=category&_expand=role`).then((res) => {
        ...
        React.$http.get("/categories").then((res) => {
            //console.log(res.data);
            setCategories(res)
        })
    ...
    const saveNewsInfo = (auditState) => {
        ...
        const p = new Promise((resovle, reject) => {
            if (props.match.params.id) {
                React.$http.patch(`/news/${newsInfo.id}`, {
                    ...formMsg,
                    "content": content,
                ...
                React.$http.post("/news?_expand=category", {
                    ...formMsg,
                    "content": content,
    ...
    return (
        <div>
            ...
            <div style={{ marginTop: "50px" }} className={current === 1 ? "" : style.hidden}>
                {/* 文本编辑器 */}
                {
                    checkEditor()
                }
            </div>
            <div style={{ marginTop: "50px" }} className={current === 2 ? "" : style.hidden}>

            </div>
            <div style={{ marginTop: "50px" }}>
                {
                    current > 0 && <span>
                        <Button onClick={lastStep} style={{ marginRight: "10px" }}>上一步</Button>
                    </span>
                }
                {
                    current < 2 && <span>
                ...
                {
                    current === 2 && <span>
                        <Button type="primary" style={{ marginRight: "10px" }} onClick={() => saveNewsInfo(0)}>保存草稿箱</Button>
                        <Button danger style={{ marginRight: "10px" }} onClick={() => saveNewsInfo(1)}>提交审核</Button>
                    </span>
                }
            </div>
